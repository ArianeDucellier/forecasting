...
def train_model(features, n_estimators, max_depth, ds, n_months, sa=False):
    """
...
def backtest(features, n_estimators, max_depth, ds_begin, ds_end, n_months, sa=False):
    """
    ...
    for i, ds in enumerate(ds_range):
        (model, scaler) = train_model(features, n_estimators, max_depth, ds, n_months, sa)
...
if __name__ == '__main__':
    ...
    for i, n_estimator in enumerate(n_estimators):
        for j, max_depth in enumerate(max_depths):
            print('SA', n_estimator, max_depth)
            (y, y_hat) = backtest(features_sa, n_estimator, max_depth, \
            ...
            error = pd.DataFrame({'n_estimators': [n_estimator], \
                'max_depths': [max_depth], 'MSE': [MSE[i, j]], \
                'RMSE': [RMSE[i, j]], 'MAE': [MAE[i, j]], \
                'MAPE': [MAPE[i, j]], 'R2': [R2[i, j]]})
            errors.append(error)
    errors = pd.concat(errors)
    errors.to_csv('XGBoost/errors_sa.csv')
    ...
    for i, n_estimator in enumerate(n_estimators):
        for j, max_depth in enumerate(max_depths):
            print('NSA', n_estimator, max_depth)
            (y, y_hat) = backtest(features_nsa, n_estimator, max_depth, \
            ...
            errors.append(error)
    errors = pd.concat(errors)
    errors.to_csv('XGBoost/errors_nsa.csv')
