...
def get_target(ds, sa=False):
    ...
    if sa == False:
        ...
        # Remove seasonality
        target_growth['target'] = remove_seasonality(target_growth['target'].to_numpy())
        return target_growth

def get_feature(name, ds, sa=False):
    """
   ...
   # Keep only national value
    ...
    if sa == False:
        ...
        # Remove seasonality
        feature[name] = remove_seasonality(feature[name].to_numpy())
        return feature

def get_train_data(name, lag, ds, sa=False):
    """
...
def train_model(name, lag, ds, sa=False):
    ...
    Input:
        ...
        sa: if True, remove seasonal component
    """
    data = get_train_data(name, lag, ds, sa)
...
def get_test_data(name, lag, ds, sa=False):
    """
    ...
    feature = get_feature(name, ds_next, sa)
    feature[name] = feature[name].shift(lag)
    df = pd.merge(target, feature, on=['date'], how='inner')
    df = df.dropna()
...
def test_model(model, name, lag, ds, sa=False):
    """
    ...
    Input:
        ...
        sa: if True, remove seasonal component
    """
    data = get_test_data(name, lag, ds, sa)
    X = data[name].to_numpy().reshape(-1, 1)
    y = data['target'].to_numpy()
    y_hat = model.predict(X)
    return (y, y_hat)

def backtest(name, lag, ds_begin, ds_end, sa=False):
    """
    ...
    for i, ds in enumerate(ds_range):
        model = train_model(name, lag, ds, sa)
        result = test_model(model, name, lag, ds, sa)
        y[i] = result[0][0]
        y_hat[i] = result[1][0]
    return (y, y_hat)
